...
def load_arguments_namespace(self, _):
    with self.argument_context('eventhubs namespace check-name-availability') as c:
...
def load_arguments_eventhub(self, _):
    with self.argument_context('eventhubs eventhub create') as c:
        ...
        c.argument('capture_size_bytes', type=int, options_list=['--capture_size-bytes'], help='The size window defines the amount of data built up in your Event Hub before an capture operation, value should be between 10485760 to 524288000 bytes')
...
def load_arguments_geodr(self, _):
    with self.argument_context('eventhubs georecovery-alias check-name-availability') as c:
        c.argument('resource_group_name', arg_type=resource_group_name_type)
        c.argument('namespace_name', options_list=['--namespace-name'], help='name of the Namespace')
        c.argument('name', options_list=['--name'],
                   help='Name of the Geo DR - Disaster Recovery Configs - Alias to check availability')
