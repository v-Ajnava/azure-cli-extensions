...
class EHNamespaceCURDScenarioTest(ScenarioTest):
    ...
    def test_eh_namespace(self, resource_group):
        ...
        self.cmd('eventhubs namespace check-name-availability --name {namespacename}',
    ...
    def test_eh_eventhub(self, resource_group):
        ...
        # Create Authoriazation Rule
        self.cmd('eventhubs eventhub authorizationrule create --resource-group {rg} --namespace-name {namespacename} --event-hub-name {eventhubname} --name {authoname} --access-rights {accessrights}',
                 checks=[self.check('name', self.kwargs['authoname'])])

        # Get Create Authorization Rule
        self.cmd('eventhubs eventhub authorizationrule show --resource-group {rg} --namespace-name {namespacename} --event-hub-name {eventhubname} --name {authoname}',
                 checks=[self.check('name', self.kwargs['authoname'])])
    ...
    def test_eh_consumergroup(self, resource_group):
        ...
        self.cmd('eventhubs consumergroup create --resource-group {rg} --namespace-name {namespacename} --event-hub-name {eventhubname} --name {consumergroupname} --user-metadata {usermetadata}',
                 checks=[self.check('name', self.kwargs['consumergroupname'])])

        # Get Consumer Group
        self.cmd('eventhubs consumergroup create --resource-group {rg} --namespace-name {namespacename} --event-hub-name {eventhubname} --name {consumergroupname}',
                 checks=[self.check('name', self.kwargs['consumergroupname'])])

        # Create/Update ConsumerGroup
        self.cmd('eventhubs consumergroup create --resource-group {rg} --namespace-name {namespacename} --event-hub-name {eventhubname} --name {consumergroupname} --user-metadata {usermetadata2}',
    ...
    def test_eh_alias(self, resource_group):
        ...
        self.cmd('eventhubs namespace check-name-availability --name {namespacenameprimary}',
                 checks=[self.check('nameAvailable', True)])

        # Create Namespace - Primary
        self.cmd('eventhubs namespace create --resource-group {rg} --name {namespacenameprimary} --location {loc_south} --tags {tags} --sku-name {sku} --sku-tier {tier}',
                 checks=[self.check('sku.name', self.kwargs['sku'])])

        # Get Created Namespace - Primary
        self.cmd('eventhubs namespace show --resource-group {rg} --name {namespacenameprimary}',
                 checks=[self.check('sku.name', self.kwargs['sku'])])

        # Create Namespace - Secondary
        self.cmd('eventhubs namespace create --resource-group {rg} --name {namespacenamesecondary} --location {loc_north} --tags {tags} --sku-name {sku} --sku-tier {tier}',
                 checks=[self.check('sku.name', self.kwargs['sku'])])

        # Get Created Namespace - Secondary
        getnamespace2result = self.cmd('eventhubs namespace show --resource-group {rg} --name {namespacenamesecondary}',
        ...
        self.cmd('eventhubs georecovery-alias check-name-availability --resource-group {rg} --namespace-name {namespacenameprimary} --name {aliasname}', checks=[self.check('nameAvailable', True)])
        ...
        # Break Pairing
        self.cmd('eventhubs georecovery-alias break-pairing  --resource-group {rg} --namespace-name {namespacenameprimary} --alias {aliasname}')

        getaliasafterbreak = self.cmd('eventhubs georecovery-alias show  --resource-group {rg} --namespace-name {namespacenameprimary} --alias {aliasname}').output
        ...
        getaliasaftercreate = self.cmd('eventhubs georecovery-alias show  --resource-group {rg} --namespace-name {namespacenameprimary} --alias {aliasname}').output
        ...
        getaliasafterfail = self.cmd('eventhubs georecovery-alias show  --resource-group {rg} --namespace-name {namespacenamesecondary} --alias {aliasname}').output
